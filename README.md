...
| **E-Mail-Eingang (externe Adresse)** | ✅ Gmail-Connector aktiv, verbunden mit `rafael.witte@froach.de` (externes Konto, getrennt von Gunnars eigenem); Routine mit Cron `0 10,16 * * *` (UTC) = 12:00/18:00 Uhr MESZ eingerichtet | Betreff-Filter seit 25.07.2026 gelockert: „Social Media"/„Socialmedia"/„SM"/„Fotos in Aktion" reichen einzeln (vorher waren alle drei Kernbegriffe zusammen nötig — das hat zwei echte Einsendungen mit dem kürzeren Betreff „Social Media" verpasst). Zum Ausgleich zählt nur noch, wer **Bildanhang** hat und nicht wie eine eigene Entwurfs-/Freigabe-Mail aussieht. **Wichtig (01.08.2026):** Gmail liefert nur Dateiname/MIME-Typ des Anhangs, nicht die Bild-Bytes selbst — es gibt keinen Attachment-Download-Tool in dieser Umgebung. Die E-Mail liefert also nur noch die Pflichtangaben als Text; das Foto selbst kommt über Google Drive (nächste Zeile). |
...
| **Google Drive — Foto-Eingang** | ✅ aktiver Foto-Kanal (seit 01.08.2026) | `download_file_content` liefert Bild-Bytes zuverlässig — anders als Gmail-Anhänge. Fotos werden in „Fotos Aktionstage" → Segment-Unterordner erwartet und per Segment + zeitlicher Nähe der passenden E-Mail zugeordnet. |
| **Google Drive — Ablageort für fertige Bilder** | ⛔ ausgeschieden | `create_file` kann keine Bild-Assets entgegennehmen (s. o.) — nur als **Eingang** aktiv, nicht als Ausgang. |
| **Drive-Ordner-Überwachung (neue/geänderte Fotos erkennen)** | ✅ eingerichtet (01.08.2026, auf Wunsch von Gunnar) | Bei jedem geplanten Lauf wird die Dateiliste der vier Segment-Unterordner gegen die gespeicherte Baseline [`state/drive-watch-baseline.json`](.claude/skills/aktionstag-post/state/drive-watch-baseline.json) abgeglichen; neue Datei-IDs gelten als frischer Eingang. Baseline muss nach jeder Verarbeitung aktualisiert + committet werden, sonst „vergisst" der nächste Lauf den Stand. |
